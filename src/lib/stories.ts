...
async function fetchEntities(): Promise<Entities> {
  let [
    ...
    client
      .listWorkflows()
      .then((wfs: Workflow[]) =>
        wfs.reduce((states, wf) => states.concat(wf.states), [])
      )
      .then(mapByItemId),
...
const fetchStories = async (program: any, entities: Entities) => {
  ...
  debug(
    'request all stories for project(s)',
    projectIds.map((p) => p.name).join(', ')
  );
  return Promise.all(
    projectIds.map((p) => client.listStories(p.id))
  ).then((projectStories) =>
...
const findProject = (entities: Entities, project: number | string) => {
  ...
  return Object.values(entities.projectsById).filter(
    (s) => !!s.name.match(projectMatch)
  )[0];
};
...
const findState = (entities: Entities, state: string | number) => {
  ...
  return Object.values(entities.statesById).filter(
    (s) => !!s.name.match(stateMatch)
  )[0];
};
...
const findEpic = (entities: Entities, epicName: string | number) => {
  ...
  return Object.values(entities.epicsById).filter((s) =>
    s.name.match(epicMatch)
  )[0];
};
...
const findIteration = (entities: Entities, iterationName: string | number) => {
  ...
  return Object.values(entities.iterationsById).filter((s) =>
    s.name.match(iterationMatch)
  )[0];
};
...
const findOwnerIds = (entities: Entities, owners: string) => {
  ...
  return Object.values(entities.membersById)
    .filter(
      (m) =>
        !!`${m.id} ${m.profile.name} ${m.profile.mention_name}`.match(
          ownerMatch
        )
    )
    .map((m) => m.id);
...
const filterStories = (program: any, stories: Story[], entities: Entities) => {
  ...
  return stories
    ...
    .filter((s) => {
      ...
      if (
        !(s.labels.map((l) => `${l.id},${l.name}`).join(',') + '').match(
          regexLabel
        )
      ) {
        return false;
      ...
      if (program.owner) {
        const owned =
          s.owners.filter((o) => {
            return !!`${o.profile.name} ${o.profile.mention_name}`.match(
              regexOwner
            );
          }).length > 0;
    ...
    log(
      format
        ...
        .replace(
          /%epic/,
          story.epic_id
            ? `${(story.epic || ({} as Epic)).name} (#${story.epic_id})`
            : '_'
        )
        ...
        .replace(
          /%i/,
          story.iteration_id
            ? `${(story.iteration || ({} as Iteration)).name} (#${
                story.iteration_id
              })`
            : '_'
        )
        ...
        .replace(
          /%s/,
          `${(story.state || ({} as WorkflowState)).name} (#${
...
const printDetailedStory = (story: StoryHydrated, entities: Entities = {}) => {
  ...
  log(
    chalk.bold('Type:') + `      ${story.story_type}/${story.estimate || '_'}`
  );
  log(chalk.bold('Label:') + `     ${labels.join(', ') || '_'}`);
  log(
    chalk.bold('Project:') +
      chalk.bold(`   #${story.project_id} `) +
      story.project.name
  );
  if (story.epic) {
    log(
      chalk.bold('Epic:') +
        chalk.bold(`      #${story.epic_id} `) +
        story.epic.name
    );
  } else {
...
const parseDateComparator: (arg: string) => (date: string) => boolean = (
  arg
) => {
  ...
  return (date) => {
    switch (comparator) {
      ...
      default:
        return (
          new Date(date.slice(0, match[0].length)).getTime() ===
          parsedDate.getTime()
        );
    }
  };
...
const parseNumberComparator: (arg: string) => (n: number) => boolean = (
  arg
) => {
  const match = arg.match(/[0-9].*/) || { index: 0, '0': { length: 30 } };
