...
const program = commander
  ...
  .option(
    '-o, --owners [id|name]',
    'Update owners of story, comma-separated',
    ''
  )
  ...
  .option(
    '--task-complete [text]',
    'Toggle completion of task on story matching text'
  )
  ...
  argIDs.concat(gitID).map(async (id) => {
    ...
    try {
      if (program.taskComplete) {
        ...
        let tasks = story.tasks.filter((t: Task) =>
          t.description.match(descMatch)
        );
        let updatedTaskIds = tasks.map((t: Task) => t.id);
        debug('request tasks complete', updatedTaskIds);
        await Promise.all(
          tasks.map((t: Task) =>
            client.updateTask(id, t.id, { complete: !t.complete })
          )
        );
